...
from slack_blocksmith.message import Message, Modal, HomeTab
from slack_blocksmith.blocks import Section, Divider, Header, Actions
from slack_blocksmith.composition import PlainText, MrkdwnText
...
def main():
    ...
    modal_section = Section.create(
        text=PlainText.create("Modal Content"),
        block_id="modal_section"
    )
    
    modal = (Modal.create("My Modal")
    ...
    home_section = Section.create(
        text=MrkdwnText.create("*Welcome to your Home Tab!*"),
        block_id="home_section"
    )
